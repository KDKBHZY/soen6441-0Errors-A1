...
public class WordstatsActor extends AbstractActor {
    ...
    private Map<String, ActorRef> wordstatsActors;
     ...
     */
    public WordstatsActor() {
        wordstatsActors = null;
        mat = null;
        hubSink = null;
        websocketFlow = null;
        injector = null;
     ...
     */
    @Inject
    public WordstatsActor(Injector injector, Materializer mat) {
        this.wordstatsActors = new HashMap<>();
        this.mat = mat;
    ...
    private void getRedditsForStats(String query) {
        ActorRef actorForQuery = wordstatsActors.get(query);
        System.out.println("update wordstats:"+query);
        if (actorForQuery != null) {
            actorForQuery.tell(new Messages.WatchwordstatsResults(query), self());
        } else {
            actorForQuery = getContext().actorOf(Props.create(GuiceInjectedActor.class, injector,
                    WordstatsResultActor.class));
            wordstatsActors.put(query, actorForQuery);
    ...
    public AbstractActor.Receive createReceive() {
        return receiveBuilder()
                .match(Messages.WatchwordstatsResults.class, watchwordstatsResults -> {
                    logger.info("!!!!Received message WatchsubRedditResults {}", watchwordstatsResults);
                    if (watchwordstatsResults != null) {
                        // Ask the searchResultsActors for a stream containing these searchResults
                        getRedditsForStats(watchwordstatsResults.query);
                        sender().tell(websocketFlow, self());
                    }
                })
                .match(Messages.UnwatchWordstatsResults.class, unwatchWordstatsResults -> {
                    logger.info("Received message UnwatchWordstatsResults {}", unwatchWordstatsResults);
                    if (unwatchWordstatsResults != null) {
...
//        return str;
     ...
     */
    public void setWordstatsActors(Map<String, ActorRef> wordstatsActors) {
        this.wordstatsActors = wordstatsActors;
    }
